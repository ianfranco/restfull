...
public class DetalleResumenCheque implements Serializable {

    private static final long serialVersionUID = 1L;
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    @Basic(optional = false)
    @Column(name = "detalle_resumen_cheque_id")
    private Integer detalleResumenChequeId;
    @Basic(optional = false)
    @NotNull
    @Column(name = "detalle_resumen_cheque_numero_cuenta")
    private int detalleResumenChequeNumeroCuenta;
    @Basic(optional = false)
    @NotNull
    @Size(min = 1, max = 45)
    @Column(name = "detalle_resumen_cheque_titular")
    private String detalleResumenChequeTitular;
    @Basic(optional = false)
    @NotNull
    @Column(name = "detalle_resumen_cheque_monto")
    private int detalleResumenChequeMonto;
    @JoinColumn(name = "detalle_resumen_cheque_id_banco", referencedColumnName = "banco_id")
    @ManyToOne(optional = false)
    private Banco detalleResumenChequeIdBanco;
    @JoinColumn(name = "detalle_resumen_cheque_id_resumen", referencedColumnName = "resumen_recaudacion_id")
    @ManyToOne(optional = false)
    ...
    public String toString() {
        return "com.areatecnica.sigf.rest.entities.DetalleResumenCheque[ detalleResumenChequeId=" + detalleResumenChequeId + " ]";
    }
